...
import ProfileQuotesList from "@/components/profile/ProfileQuotesList";
...
export default async function ProfileQuotesPage({ params }: Props) {
  ...
  const quotesResult = await getPublicQuotesByUsername(username, viewer?.id, {
    limit: 12,
    offset: 0,
  });
  ...
  const initialHasMore =
    quotesResult.found && !quotesResult.isPrivate ? quotesResult.hasMore : false;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-border/40">
            ...
            <div className="text-right sm:text-left max-w-md">
              <p className="text-xs leading-6 text-muted-foreground sm:text-sm">
                مجموعه جملات و بخش‌های به یادماندنی از کتابخانه شخصی {profile.displayName || profile.username}
              </p>
            </div>
          </div>
