...
test("successful commit replaces review UI and advances stepper to terminal stage", () => { assert.match(review, /if \(commitResult\)[\s\S]*IranKetabImportSuccess/); assert.match(review, /commitResult \? 6/); assert.match(preview, /terminalSuccess \? 5/); });
