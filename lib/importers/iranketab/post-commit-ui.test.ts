...
test("successful commit replaces review UI and advances stepper to terminal stage", () => { assert.match(review, /if \(commitResult\) return <IranKetabImportSuccess/); assert.match(review, /commitResult \? 5/); assert.match(preview, /terminalSuccess \? 5/); });
...
test("autosave and recovery cannot overwrite terminal success", () => { assert.match(review, /terminalRef\.current \|\| commitResult/); assert.match(preview, /if \(!terminalSuccessRef\.current\) setRecoverable/); });
