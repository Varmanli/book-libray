...
export function initializeIranKetabDraft(
  ...
  const profileFor = (type: z.infer<typeof entityType>, name: string) => {
    ...
    return profile ? { profileId: profile.profileId ?? null, originalName: profile.originalName, slug: profile.slug, sourceUrl: profile.sourceUrl, description: profile.description, imageUrl: profile.imageUrl, bannerImageUrl: profile.bannerImageUrl, birthYear: profile.birthYear, deathYear: profile.deathYear, countryName: profile.country?.name ?? null, countrySlug: profile.country?.slug ?? null, website: profile.website, seoTitle: profile.seoTitle, seoDescription: profile.seoDescription, metadata: profile.metadata } : undefined;
  };
