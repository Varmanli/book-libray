...
function analyzeEdition(extraction: IranKetabExtractionEnvelope, edition: IranKetabExtractionEnvelope["editions"][number], extractedIndex: number, rows: AnalysisEditionRow[], selectedCatalogId: string | null, conflicts: AnalysisIssue[], warnings: AnalysisIssue[]): EditionAnalysis {
  ...
  const byIsbn = rows.filter(row => (isbn13.value && normalizeIsbn(row.isbn13) === isbn13.value) || (isbn10.value && normalizeIsbn(row.isbn10) === isbn10.value));
  const candidates = unique([...byCode,...byIsbn]);
  const strong = candidates.filter(row => byCode.includes(row) || byIsbn.includes(row));
  ...
  else if (strong.length === 1) { chosen=strong[0]; const incompatible=selectedCatalogId && chosen.catalogBookId!==selectedCatalogId; if(incompatible){status="CONFLICT";confidence="EXACT";reasons.push("ISBN یا کد منبع متعلق به کاتالوگ دیگری است.");conflicts.push(issue("EDITION","CRITICAL",`شناسه نسخه ${edition.sourceEditionCode} به کتاب دیگری تعلق دارد.`,true,"DO_NOT_OVERWRITE",extractedIndex));} else {status="EXACT_MATCH";confidence="EXACT";reasons.push(byCode.includes(chosen)?"کد نسخه ایران‌کتاب تطبیق دقیق دارد.":"شابک تطبیق دقیق دارد.");} }
